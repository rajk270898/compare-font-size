  ...
  it('should test all URLs', () => {
    cy.fixture('urls.json').then(urlsData => {
      ...
      function processNextUrl() {
        ...
        function processViewport(viewportIndex) {
          if (viewportIndex >= viewports.length) {
            // All viewports done, write results and move to next URL
          ...
          const view = viewports[viewportIndex];
          cy.log(`Setting viewport: ${view.name} (${view.width}x${view.height})`);
          
          // Set viewport and verify it was set correctly
          cy.viewport(view.width, view.height);
          cy.window().then((win) => {
            const actualWidth = win.innerWidth;
            const actualHeight = win.innerHeight;
            cy.log(`Actual viewport size: ${actualWidth}x${actualHeight}`);
            if (actualWidth !== view.width || actualHeight !== view.height) {
              cy.log(`⚠️ Warning: Viewport size mismatch. Expected: ${view.width}x${view.height}, Got: ${actualWidth}x${actualHeight}`);
            }
          });
          ...
          // Process selectors in smaller chunks to save memory
          function processSelectorChunk(startIdx, chunkSize) {
            const endIdx = Math.min(startIdx + chunkSize, selectors.length);
            const currentChunk = selectors.slice(startIdx, endIdx);

            if (startIdx >= selectors.length) {
              cy.log(`Completed viewport ${view.name} (${view.width}x${view.height}) for ${urlData.name}`);
              // Move to next viewport
              processViewport(viewportIndex + 1);
              return;
            }

            cy.wrap(currentChunk).each((selector, idx) => {
              const actualIdx = startIdx + idx;
              cy.log(`Processing selector ${actualIdx + 1}/${selectors.length}: ${selector}`);
              ...
              // Process next chunk of selectors
              processSelectorChunk(endIdx, chunkSize);
            });
          }

          // Start processing selectors in chunks of 10
          processSelectorChunk(0, 10);
        }

        // Start with first viewport (4k Screen)
        processViewport(0);
      }
